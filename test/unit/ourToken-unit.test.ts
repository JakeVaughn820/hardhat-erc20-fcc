...
describe("OurToken Unit Test", function () {
  let ourToken: OurToken, deployer: SignerWithAddress, user1: SignerWithAddress
  ...
  it("Should be able to transfer tokens successfully to an address", async function () {
    ...
    const user1TokenBalance = await ourToken.balanceOf(user1.address)

    assert.equal(user1TokenBalance.toString(), tokensToSend.toString())
  })
  ...
  it("Should approve other address to spend token", async () => {
    ...
    const user1SpendAmount = await ourToken1.balanceOf(user1.address)
    assert.equal(user1SpendAmount.toString(), tokensToSpend.toString())
  })
})
